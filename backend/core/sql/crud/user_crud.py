...
class CRUDUser:
    ...
    def update(self, **kwargs):
        ...
        """
        try:
            logging.info("CRUDUser update request")
            with session() as transaction_session:
                obj: User = (
                    transaction_session.query(User)
                    .filter(User.user_name == kwargs.get("user_name"))
    ...
    def update_role(self, **kwargs):
        ...
        try:
            ...
            with session() as transaction_session:
                obj: User = (
                    transaction_session.query(User)
                    .filter(User.role_id == kwargs.get("role_id"))
                    .update(kwargs, synchronize_session=False)
                )
                transaction_session.commit()
        except Exception as error:
    ...
    def delete(self, user_name: str):
        ...
        """
        try:
            logging.info("CRUDUser delete request")
            with session() as transaction_session:
                obj: User = (
                    transaction_session.query(User)
                    .filter(User.user_name == user_name)
