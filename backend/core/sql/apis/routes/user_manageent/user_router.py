...
def forgot_password(
    ...
    """
    try:
        logging.info("Calling /user/forgot_password endpoint")
        logging.debug(f"Request: {forgot_pass_request}")
        status = UserManagementController().forgot_password_controller(
            request=forgot_pass_request
        )
        return status
    except Exception as error:
        logging.error(f"Error in /user/forgot_password endpoint: {error}")
        raise error
...
def update_password(
    ...
    """
    try:
        logging.info("Calling /user/update_password endpoint")
        logging.debug(f"Request: {update_pass_request}")
        status = UserManagementController().update_password_controller(
            request=update_pass_request
        )
        return status
    except Exception as error:
        logging.error(f"Error in /user/update_password endpoint: {error}")
        raise error
...
async def update_user_role(
    ...
    """
    try:
        logging.info("Calling /user/update/role endpoint")
        authenticated_user_details = decodeJWT(token=token)
        if authenticated_user_details:
            _ = check_user_authorization(
                user_details=authenticated_user_details, allowed_user_role_id=1
...
async def delete_user(
    ...
    try:
        ...
        if authenticated_user_details:
            ...
            return UserManagementController().delete_user_controller(
                user_name=user_name
            )
        else:
            raise HTTPException(
        ...
        logging.error(f"Error in /user/delete endpoint: {error}")
        raise error
...
async def get_all_users():
    ...
    """
    try:
        logging.info("Calling /user/list_all endpoint")
        list_of_users = UserManagementController().get_all_users_controller()
        return list_of_users
    except Exception as error:
        logging.error(f"Error in /user/list_all endpoint: {error}")
        raise error
...
async def get_user_details(
    ...
    try:
        logging.info("Calling /user/user_details endpoint")
        user_details = UserManagementController().get_user_controller(
            user_name=user_name
        )
        return UserDetails(**user_details)
    except Exception as error:
        logging.error(f"Error in /user/user_details endpoint: {error}")
        raise error
