...
@app_config_router.post(
    "/accuracy_packaging_server/app_config/update",
...
async def update_app_config(
    ...
    """
    try:
        logging.info("Calling /accuracy_packaging_server/app_config/update endpoint")
        logging.debug(f"Request: {update_app_config_request}")
        ...
        logging.error(
            f"Error in /accuracy_packaging_server/app_config/update endpoint: {error}"
        )
        raise HTTPException(
...
@app_config_router.post(
    "/accuracy_packaging_server/app_config/create",
    response_model=UpdateConfigResponse,
)
async def create_app_config(
    ...
    try:
        logging.info("Calling /accuracy_packaging_server/app_config/update endpoint")
        logging.debug(f"Request: {create_app_config_request}")
        authenticated_user_details = decodeJWT(token=token)
        if authenticated_user_details:
            _ = check_user_authorization(
                user_details=authenticated_user_details, allowed_user_role_id=1
        ...
        logging.error(
            f"Error in /accuracy_packaging_server/app_config/create endpoint: {error}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(error)
        )
...
@app_config_router.get(
    "/accuracy_packaging_server/app_config/list_all",
)
async def list_all_app_config(
    ...
    """
    try:
        logging.info("Calling /accuracy_packaging_server/app_config/list_all endpoint")
        ...
        logging.error(
            f"Error in /accuracy_packaging_server/app_config/list_all endpoint: {error}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(error)
        )
...
@app_config_router.get(
    "/accuracy_packaging_server/app_config/get_config",
    response_model=UpdateConfigResponse,
)
async def get_app_config(
    ...
    """
    try:
        logging.info("Calling /accuracy_packaging_server/app_config/get_config endpoint")
        if decodeJWT(token=token):
        ...
        logging.error(
            f"Error in /accuracy_packaging_server/app_config/get_config endpoint: {error}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(error)
        )
...
@app_config_router.delete("/accuracy_packaging_server/app_config/delete")
async def delete_cloud(
    ...
    """
    try:
        logging.info("Calling /accuracy_packaging_server/app_config/delete endpoint")
        authenticated_user_details = decodeJWT(token=token)
        if authenticated_user_details:
            _ = check_user_authorization(
                user_details=authenticated_user_details, allowed_user_role_id=1
