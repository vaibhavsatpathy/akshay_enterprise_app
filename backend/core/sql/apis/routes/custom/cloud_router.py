...
@cloud_router.post(
    "/accuracy_packaging_server/cloud/create",
...
async def create_cloud_config(
    ...
    """
    try:
        logging.info("Calling /accuracy_packaging_server/cloud_config/create endpoint")
        logging.debug(f"Request: {create_cloud_request}")
        ...
        logging.error(
            f"Error in /accuracy_packaging_server/cloud_config/create endpoint: {error}"
        )
        raise HTTPException(
...
@cloud_router.post(
    "/accuracy_packaging_server/cloud/update",
...
async def update_cloud(
    ...
    """
    try:
        logging.info("Calling /accuracy_packaging_server/cloud/update endpoint")
        logging.debug(f"Request: {update_cloud_request}")
        authenticated_user_details = decodeJWT(token=token)
        if authenticated_user_details:
            _ = check_user_authorization(
                user_details=authenticated_user_details, allowed_user_role_id=1
        ...
        logging.error(f"Error in /accuracy_packaging_server/cloud/update endpoint: {error}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(error)
        )
...
@cloud_router.get(
    "/accuracy_packaging_server/cloud/list_all",
)
async def list_all_cloud(
    ...
    """
    try:
        logging.info("Calling /accuracy_packaging_server/cloud/list_all endpoint")
        ...
        logging.error(
            f"Error in /accuracy_packaging_server/cloud/list_all endpoint: {error}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(error)
        )
...
@cloud_router.get(
    "/accuracy_packaging_server/cloud/get_cloud",
    response_model=UpdateCloudNameResponse,
)
async def get_cloud_config(
    ...
    """
    try:
        logging.info("Calling /accuracy_packaging_server/cloud/get_cloud endpoint")
        if decodeJWT(token=token):
        ...
        logging.error(
            f"Error in /accuracy_packaging_server/cloud/get_cloud endpoint: {error}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(error)
        )
...
@cloud_router.delete("/accuracy_packaging_server/cloud/delete")
async def delete_cloud(
    ...
    """
    try:
        logging.info("Calling /accuracy_packaging_server/delete endpoint")
        authenticated_user_details = decodeJWT(token=token)
        if authenticated_user_details:
            _ = check_user_authorization(
                user_details=authenticated_user_details, allowed_user_role_id=1
        ...
        logging.error(f"Error in /accuracy_packaging_server/delete endpoint: {error}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(error)
        )
